...
mod tests {
    ...
    #[test]
    fn grant_profiles_preserve_the_broker_wire_contract() {
        for (profile, number, rights) in [
            (GrantProfile::DisplayV1, 1, 0x0f),
            (GrantProfile::DisplayCecV1, 2, 0x1f),
            (GrantProfile::DisplayCecAudioV1, 3, 0x3f),
        ] {
            assert_eq!(profile as u16, number);
            assert_eq!(profile.rights(), rights);
        }
    }
