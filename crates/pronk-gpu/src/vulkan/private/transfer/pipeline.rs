...
impl Program {
    pub(super) fn new(device: Arc<DeviceInner>, function: Function) -> io::Result<Self> {
        ...
        // SAFETY: The checked-in Vulkan 1.1 shader matches the descriptor and
        // push-constant layouts and requires no optional device feature.
        unsafe {
            owner.layout = owner
                .device
                .raw
...
fn requirements(device: &DeviceInner) -> io::Result<vk::PhysicalDeviceLimits> {
    ...
    {
        return Err(unsupported("sRGB transfer compute program is unsupported"));
    }
    Ok(limits)
}
...
pub(super) fn check_support(device: &DeviceInner, extent: Extent) -> io::Result<()> {
    ...
    {
        return Err(unsupported("sRGB transfer dispatch is unsupported"));
    }
    Ok(())
}
