...
fn private_image_copies_survive_source_rewrite() {
    ...
    for rgb in [[255, 0, 0], [0, 255, 0], [0, 0, 255], [17, 85, 204]] {
        source = source.clear_and_wait(rgb).unwrap().0;
        let copied = destination.copy_from_and_wait(source).unwrap();
        assert_eq!(
            copied.completion.wait_blocking().unwrap(),
            Completion::Success
        );
        source = copied.source.clear_and_wait([255, 255, 255]).unwrap().0;
...
fn copies_reject_undefined_sources_or_incompatible_devices_and_extents() {
    ...
    assert!(
        matches!(allocate().copy_from_and_wait(allocate()), Err(error) if error.kind() == io::ErrorKind::InvalidInput)
    );
    let initialized = || allocate().clear_and_wait([1, 2, 3]).unwrap().0;
    let other_size = NonZeroU32::new(32).unwrap();
    let destination = device.allocate(other_size, size, modifier).unwrap();
    assert!(
        matches!(destination.copy_from_and_wait(initialized()), Err(error) if error.kind() == io::ErrorKind::InvalidInput)
    );
