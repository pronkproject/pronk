...
async fn main() -> anyhow::Result<()> {
    let args: Vec<_> = std::env::args().skip(1).collect();
        ...
        args.len() == 4,
        "expected device, CRTC, connector and private socket in a disposable Mutter session"
    );
...
async fn run(target: Target, socket: &Path) -> anyhow::Result<()> {
    ...
    let mut last_timestamp = None;
    {
        ...
        while decoded < 12 || colors.len() < 2 || !started {
            tokio::select! {
